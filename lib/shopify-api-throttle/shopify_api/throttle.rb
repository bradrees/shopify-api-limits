  ...
  module Throttle
    module ClassMethods
      ...
      def throttle(&block)
        retried ||= 0
        begin
          if credit_below_safe?(THROTTLE_MIN_CREDIT)
            ...
            if $shopify_store && $shopify_store.respond_to?(:on_throttled) # Use this to call back into your application when throttled
              $shopify_store.on_throttled
            end
          end

          if $shopify_store && $shopify_store.respond_to?(:on_request) # Use this to call back into your application when a request happens
            $shopify_store.on_request
          end
